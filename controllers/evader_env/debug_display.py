...
class DebugDisplayMixin:
    def _draw_reward_label(self, reward: float, info: dict[str, float | bool]) -> None:
        ...
        text = (
            ...
            f"risk {info['front_corridor_risk']:.2f} safe {int(info['obstacle_safety_active'])}/{info['obstacle_safety_action_delta']:.2f} pAv {int(info['pursuer_avoidance_active'])}/{int(info['pursuer_avoidance_obstacle_count'])} "
